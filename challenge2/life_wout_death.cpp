...
int parallel_step(Grid &cur, Grid &next) {
    ...
    #pragma omp parallel for collapse(2) reduction(|:changes) schedule(dynamic) num_threads(16)
    for(int y = 0; y < cur.H; y += tile_y) {
        for(int x = 0; x < cur.W; x += tile_x) {
            for(int ty = 0; ty < tile_y; ++ty) {
                ...
                for(int tx = 0; tx < tile_x; ++tx) {
                    ...
                    for(int dy = -1; dy <= 1; ++dy) {
                        for(int dx = -1; dx <= 1; ++dx) {
                            if(dx == 0 && dy == 0) continue;
                            int nx = (cx + dx + cur.W) % cur.W;
                            int ny = (cy + dy + cur.H) % cur.H;
